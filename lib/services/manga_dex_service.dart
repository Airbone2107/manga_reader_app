...
class MangaDexService {
  ...
  // Lấy danh sách manga
  Future<List<dynamic>> fetchMangaList({required int limit, required int offset}) async {
    final response = await http.get(Uri.parse('$baseUrl/manga?limit=$limit&offset=$offset'));
    ...
    if (response.statusCode == 200) {
      ...
      logError('fetchMangaList', response); // Ghi log lỗi nếu không phải mã trạng thái 200 hoặc 503
      throw Exception('Lỗi khi tải manga với mã trạng thái: ${response.statusCode}');
    }
  }
